...
class SkipCardTest extends ActionCardTest {

    @Override
    @BeforeEach
    protected void setUp() {
        concreteCard = new SkipCard(Color.RED);
    }

    @Test
    protected void getFaceValue() {
        assertEquals("Skip", concreteCard.getFaceValue());
    }
    ...
    protected void canPlaceOn() {
        ...
        for (Color color : Color.values()) {
            SkipCard mockCard = Mockito.mock(SkipCard.class);
            Mockito.when(mockCard.getFaceValue()).thenReturn("Skip");
    ...
    protected void testEquals() {
        ...
        SkipCard equalSkipCard = new SkipCard(Color.RED);
        SkipCard unequalSkipCard = new SkipCard(Color.BLUE);

        assertEquals(concreteCard.equals(equalSkipCard), equalSkipCard.equals(concreteCard));
        assertEquals(concreteCard.equals(unequalSkipCard), unequalSkipCard.equals(concreteCard));

        // Transitivity: if x.equals(y) and y.equals(z) then also x.equals(z)
        SkipCard y = new SkipCard(Color.RED);
        SkipCard z = new SkipCard(Color.RED);
    ...
    protected void testHashCode() {
        ...
        assertEquals(equalSkipCard.hashCode(), concreteCard.hashCode());
    }
}
